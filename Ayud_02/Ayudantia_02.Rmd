...
library(readxl)        ## Carga datos tipo xlsx
```
...
```{r sapply}
sapply(mas_dias, FUN = unique) # UNIQUE TRAE LOS VALORES NO REPETIDO DE LA VARIABLE
```
...
#CBIND, SUMA COLUMNAS, UNA AL LADO DE LA OTRA, LO QUE HACE SI UNA BASDE TIENE MENOS OBS QUE LA OTRA, TE REPITE LOS DATOS
# RBIND HACE SUMAR FILAS (PONE UNA BASE DE DATO SOBRE LA OTRA, QUEDA UNA CON LA MISMA CANT DE COLUMNAS PERO CON MAS OBSERVACIONES)
#MERGE, TRABAJA CON BASES DE DATOS DE DISTINTOS TAMAÑOS, SI FALTAN DATOS, LOS ASIGNA COMO NA

```{r cbind}
dim(casos)[1] == dim(muertes)[1] # ver si tienen el mismo numero de filas
cbind(casos,muertes[,.(deaths)]) # Para que no se repitan las columnas, nos quedamos solo con la columna "deaths" de "muertes".
```
# cbind, no reconoce q muertes van con q pais
...
Aquí lo que indico a R es que me adjunte los datos de las columnas de la segunda base de datos a la primera, de acuerdo a que ambas filas (de ambas bases de datos) cumplan con los mismos elementos en la columna "day", "month", "year" y "countriesAndTerritories". Ojo que, en caso de tener más de una fila que cumpla con dichas categorías, entonces hagran problemas en el merge (R no tiene instrucciones de a que fila asignar dicho dato repetido). En ese caso sería bueno hacer merge por más condiciones o revisar si la base de datos tiene algún error.
...
Anteriormente observamos que en la variable "countryterritoryCode" existen valores NA. ¿Pero como corroborar para el resto de las variables si también existen NAs? ¿Y para las otras bases?
...
```{r % NAs}
apply(al_29_03_2020, 2, function(col){paste0(round(sum(is.na(col))/length(col),5)*100,"%")})
```
...
Supongamos que deseamos sacar la suma de todos los muertos y casos por mes y país, así como el promedio de la población. Eso se puede hacer en base a un colapso de la base "chileyvecinos_completa":
...
```{r crear variables} # creo una varible y la agrego a la base de datos
